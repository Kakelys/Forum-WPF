    ...
    public class HtmlUpdater
    {
        ...
        private Inline UpdateElement(HtmlTag aTag)
        {
            ...
            switch (aTag.Name)
            {
                case "img":
                {
                    ...
                    var img = new Image();
                        ...
                        var url = currentState.ImgLink ?? "";

                        bitmap.UriSource = new System.Uri(url, System.UriKind.RelativeOrAbsolute);
                        bitmap.EndInit();

                        img.Source = bitmap;
                        img.Height = url==""? 0 : (double)currentState.ImgHeight;
                        img.Width = url==""? 0 : (double)currentState.ImgWidth;
            ...
            if (currentState.HyperLink != null && currentState.HyperLink != "")
            {
                ...
                try
                {
                    link.RequestNavigate += HyperlinkNavigate;
